...
export const cinematicBarsMaskDefinition: MaskDefinition<RectangleMaskParams> =
	{
		...
		buildOverlayPath({ width, height }) {
			const bandHeight = Math.max(height * 0.18, 8);
			const bottomY = Math.max(height - bandHeight, bandHeight);
			return [
				`M 0,0 H ${width} V ${bandHeight} H 0 Z`,
				`M 0,${bottomY} H ${width} V ${height} H 0 Z`,
			].join(" ");
		},
		features: {
			hasPosition: true,
